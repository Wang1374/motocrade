...
public class WxAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
     ...
     */
    private static final String SPRING_SECURITY_FORM_WX_CODE = "code";

    /**
     * 微信登录的iv参数
     */
    private static final String SPRING_SECURITY_FORM_WX_IV = "iv";

    /**
     * 微信登录的encryptedData参数
     */
    private static final String SPRING_SECURITY_FORM_WX_ENCRYPTEDDATA = "encryptedData";

    /**
     * 微信用户的基本信息 昵称，头像等
     */
    private static final String SPRING_SECURITY_FORM_WX_WXINFO = "wxInfo";

    @Getter
    @Setter
    private String wxParameterEncry = SPRING_SECURITY_FORM_WX_ENCRYPTEDDATA;


    @Getter
    @Setter
    private String wxParameterIV = SPRING_SECURITY_FORM_WX_IV;

    @Getter
    @Setter
    private String wxParameterWxInfo = SPRING_SECURITY_FORM_WX_WXINFO;


    @Getter
    @Setter
    private String wxParameter = SPRING_SECURITY_FORM_WX_CODE;
    ...
    private void setWxUserDetails(HttpServletRequest request, WxAuthenticationToken authRequest) {
        try {
            ...
            String iv = request.getParameter(wxParameterIV);
            String encryptedData = request.getParameter(wxParameterEncry);
            String wxInfo = request.getParameter(wxParameterWxInfo);
            WxUser wxData = JSONObject.parseObject(wxInfo, WxUser.class);

            if (StringUtils.isNotBlank(iv) && StringUtils.isNotBlank(encryptedData)) {
                WxUser user = new WxUser();
                if(wxData!=null)
                    BeanUtils.copyProperties(wxData,user);
//                String encodeIv = URLEncoder.encode(iv, "UTF-8");
                user.setIv(iv);
                user.setEncryptedData(encryptedData);
                authRequest.setWxUser(user);
            }
        } catch (Exception e) {
            log.error("设置微信用户信息失败",e);
        }
