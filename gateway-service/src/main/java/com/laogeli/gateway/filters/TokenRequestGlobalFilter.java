...
public class TokenRequestGlobalFilter implements GlobalFilter, Ordered {
    ...
    /**
     * refresh_token超时时间，默认1天
     */
    private static long REFRESH_TOKEN_EXPIRE = 60 * 60 * 24 * 2;
            ...
            public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
                if (getStatusCode() != null && getStatusCode().equals(HttpStatus.OK) && body instanceof Flux) {
                    ...
                    return super.writeWith(fluxBody.buffer().map(dataBuffers -> {
                        ...
                        originalResponse.getHeaders().setContentLength(converted.getBytes().length);
    ...
    private boolean match(ServerHttpRequest request) {
        return HttpMethod.POST.matches(request.getMethodValue())
                && StrUtil.containsAnyIgnoreCase(request.getURI().getPath(), GatewayConstant.OAUTH_TOKEN_URL, GatewayConstant.MOBILE_TOKEN_URL,GatewayConstant.WX_URL);
    }
    ...
    private boolean matchGrantType(String grantType) {
        return CommonConstant.GRANT_TYPE_PASSWORD.equals(grantType)
                || CommonConstant.GRANT_TYPE_MOBILE.equals(grantType)
                || GatewayConstant.GRANT_TYPE_REFRESH_TOKEN.equals(grantType)
                || GatewayConstant.WX_TOKEN_URL.equalsIgnoreCase(grantType);
    }
}
